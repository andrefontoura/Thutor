    ...
    init: function (editor) {
       ...
       function counterId(editorInstance) {
        ...
        function limitReached(editorInstance, notify) {
            ...
            if (config.hardLimit) {
                //editorInstance.loadSnapshot(snapShot); //Originalmente habilitado
                // lock editor
        ...
        function updateCounter(editorInstance) {
			...
			if ((config.maxWordCount > -1 && wordCount > config.maxWordCount && deltaWord > 0) ||
                ...
                //limitReached(editorInstance, limitReachedNotified); //Original
				limitRestored(editorInstance);
            } else if ((config.maxWordCount == -1 || wordCount <= config.maxWordCount) &&
            (config.maxCharCount == -1 || charCount <= config.maxCharCount)) {

                //limitRestored(editorInstance); //Original
            ...
            if (config.maxWordCount > 0 || config.maxCharCount > 0) {
                ...
                if (config.maxCharCount > 0 && charCount > config.maxCharCount && config.hardLimit) {
                    if(!notification.isVisible()) {
                        //notification.show(); //Original
                    }
                   // event.cancel();//Original
                }
